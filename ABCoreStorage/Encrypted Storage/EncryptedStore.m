...
typedef void (*sqlite3_context_func_t)(sqlite3_context*,int,sqlite3_value**);
...
+ (NSPersistentStoreCoordinator *)makeStoreWithDatabaseURL:(NSURL *)databaseURL managedObjectModel:(NSManagedObjectModel *)objModel :(NSString*)passcode
{
	...
	if (error)
	{
		DLog(@"Unable to add persistent store\n%@", error);
		return nil;
	}
    ...
    if ([request requestType] == NSFetchRequestType) {
        ...
        if (type == NSManagedObjectResultType || type == NSManagedObjectIDResultType) {
            ...
            while (sqlite3_step(statement) == SQLITE_ROW) {
                sqlite3_int64 primaryKey = sqlite3_column_int64(statement, 0);
                NSEntityDescription * entityToFecth = nil;
                if (shouldFetchEntityType) {
                    int entityType = sqlite3_column_int(statement, 1);
        ...
        if (type == NSDictionaryResultType && [[fetchRequest propertiesToFetch] count] > 0) {
            BOOL isDistinctFetchEnabled_ = [fetchRequest returnsDistinctResults];
            ...
            NSString *string = [NSString stringWithFormat:
                                @"SELECT %@%@ FROM %@ %@%@%@%@;",
                                (isDistinctFetchEnabled_)?@"DISTINCT ":@"",
...
- (NSIncrementalStoreNode *)newValuesForObjectWithID:(NSManagedObjectID *)objectID
    ...
    sqlite3_int64 primaryKey = [(NSNumber*)[self referenceObjectForObjectID:objectID] longLongValue];
    // run query
...
- (id)newValueForRelationship:(NSRelationshipDescription *)relationship
    ...
    // prepare values
    sqlite3_int64 key = [(NSNumber*)[self referenceObjectForObjectID:objectID] longLongValue];
...
- (void)managedObjectContextDidRegisterObjectsWithIDs:(NSArray *)objectIDs {
    ...
    [objectIDs enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if (strongSelf) {
            NSUInteger value = [(NSNumber*)[strongSelf->objectCountCache objectForKey:obj] unsignedIntegerValue];
            [strongSelf->objectCountCache setObject:@(value + 1) forKey:obj];
        }
    }];
}
...
- (void)managedObjectContextDidUnregisterObjectsWithIDs:(NSArray *)objectIDs {
    ...
    [objectIDs enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        __strong typeof(self) strongSelf = weakSelf;
        if (strongSelf) {
            NSNumber *value = [strongSelf->objectCountCache objectForKey:obj];
            if (value) {
                NSUInteger newValue = ([value unsignedIntegerValue] - 1);
                if (newValue == 0) {
                    [strongSelf->objectCountCache removeObjectForKey:obj];
                    [strongSelf->nodeCache removeObjectForKey:obj];
                }
                else { [strongSelf->objectCountCache setObject:@(newValue) forKey:obj]; }
            }
        }
    }];
}
...
- (BOOL)loadMetadata:(NSError * __autoreleasing *)error {
    if (sqlite3_open([[[self URL] path] UTF8String], &database) == SQLITE_OK) {
        ...
        BOOL success = [self performInTransaction:^{
            ...
            sqlite3_create_function(strongSelf->database, "REGEXP", 2, SQLITE_ANY, NULL, (sqlite3_context_func_t)dbsqliteRegExp, NULL, NULL);
            ...
            if (hasTable) {
                ...
                if ([[options objectForKey:NSMigratePersistentStoresAutomaticallyOption] boolValue] &&
                    ...
                    NSManagedObjectModel *newModel = [persistentStoreCoordinator managedObjectModel];
            ...
            // this is a new store
                ...
                // run migrations
                NSPersistentStoreCoordinator *persistentStoreCoordinator = self.persistentStoreCoordinator;
                NSManagedObjectModel *model = [persistentStoreCoordinator managedObjectModel];
...
- (BOOL)hasMetadataTable:(BOOL *)hasTable error:(NSError * __autoreleasing *)error {
...
- (BOOL)initializeDatabaseWithModel:(NSManagedObjectModel *)model error:(NSError* __autoreleasing *)error {
...
- (BOOL)createTableForEntity:(NSEntityDescription *)entity error:(NSError* __autoreleasing *)error {
...
- (BOOL)createIndicesForEntity:(NSEntityDescription *)entity error:(NSError * __autoreleasing *)error
{
    if (entity.superentity) {
        return YES;
    }
...
- (NSArray *)handleSaveChangesRequest:(NSSaveChangesRequest *)request error:(NSError * __autoreleasing *)error {
...
- (BOOL)handleInsertedObjectsInSaveRequest:(NSSaveChangesRequest *)request error:(NSError * __autoreleasing *)error {
    BOOL __block success = YES;
    [[request insertedObjects] enumerateObjectsUsingBlock:^(NSManagedObject *object, BOOL *stop) {
        ...
        NSDictionary *properties = [entity propertiesByName];
        [properties enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *shouldStop) {
            if ([obj isKindOfClass:[NSAttributeDescription class]]) {
                [keys addObject:key];
        ...
        // bind id
        sqlite3_int64 number = [(NSNumber*)[self referenceObjectForObjectID:[object objectID]] longLongValue];
        sqlite3_bind_int64(statement, 1, number);
        
        // bind properties
        [keys enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *shouldStop) {
            NSPropertyDescription *property = [properties objectForKey:obj];
            @try {
                [self
...
- (BOOL)handleUpdatedObjectsInSaveRequest:(NSSaveChangesRequest *)request cache:(NSMutableDictionary *)cache error:(NSError * __autoreleasing *)error {
    BOOL __block success = YES;
...
#define USE_MANUAL_NODE_CACHE 1
        ...
        NSDictionary *properties = [entity propertiesByName];
        [changedAttributes enumerateKeysAndObjectsUsingBlock:^(id key, id value, BOOL *shouldStop) {
        ...
        // execute
        sqlite3_int64 number = [(NSNumber*)[self referenceObjectForObjectID:[object objectID]] longLongValue];
        sqlite3_bind_int64(statement, (int)([columns count] + 1), number);
        sqlite3_step(statement);
...
- (BOOL)handleDeletedObjectsInSaveRequest:(NSSaveChangesRequest *)request error:(NSError * __autoreleasing *)error {
    BOOL __block success = YES;
    [[request deletedObjects] enumerateObjectsUsingBlock:^(NSManagedObject *object, BOOL *shouldStop) {
        ...
        NSString *string = [NSString stringWithFormat:
                            ...
                            [self tableNameForEntity:entity]];
        sqlite3_stmt *statement = [self preparedStatementForQuery:string];
        sqlite3_bind_int64(statement, 1, [objectID longLongValue]);
        sqlite3_step(statement);
...
- (BOOL)hasTable:(BOOL *)hasTable withName:(NSString*)name error:(NSError * __autoreleasing *)error {
...
- (void) maybeAddJoinStatementsForKey: (NSString *) key
    ...
    for (NSUInteger i = 0 ; i < keysArray.count - 1; i++) {
    ...
    if (value && ![value isKindOfClass:[NSNull class]]) {
        if ([property isKindOfClass:[NSAttributeDescription class]]) {
            ...
            // blob
                ...
                sqlite3_bind_blob(statement, index, [value bytes], (int)[(NSData*)value length], SQLITE_TRANSIENT);
            }
            ...
            sqlite3_int64 number = [(NSNumber*)[self referenceObjectForObjectID:[value objectID]] longLongValue];
...
- (NSDictionary *)recursiveWhereClauseWithFetchRequest:(NSFetchRequest *)request predicate:(NSPredicate *)predicate {
    ...
    if ([predicate isKindOfClass:[NSCompoundPredicate class]]) {
        ...
        if (rightOperand && !rightBindings) {
            ...
            NSMutableArray *queryChunks = [NSMutableArray arrayWithArray:@[leftOperand, [operator objectForKey:@"operator"], rightBindings]];
            
            if (rightOperand && [rightOperand isKindOfClass:[NSString class]] && [rightOperand isEqualToString:@"? COLLATE NOCASE"])
            {
                [queryChunks addObject:@"COLLATE NOCASE"];
            }
            
            query = [queryChunks componentsJoinedByString:@" "];
    ...
    if (request.entity.superentity != nil) {
        if (request.entity.subentities.count > 0 && request.includesSubentities) {
            ...
            entityWhere = [NSString stringWithFormat:@"%@._entityType = %u",
                           [self tableNameForEntity:request.entity],
                           (unsigned)request.entityName.hash];
        }
...
- (void)bindWhereClause:(NSDictionary *)clause toStatement:(sqlite3_stmt *)statement {
    ...
    [bindings enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            sqlite3_bind_text(statement, (int)(idx + 1), [obj UTF8String], -1, SQLITE_TRANSIENT);
        }
        ...
        // managed object id
            ...
            sqlite3_int64 number = [(NSNumber*)[self referenceObjectForObjectID:obj] longLongValue];
            sqlite3_bind_int64(statement, (int)(idx + 1), number);
        }
        
        // managed object
        else if ([obj isKindOfClass:[NSManagedObject class]]) {
            sqlite3_int64 number = [(NSNumber*)[self referenceObjectForObjectID:[obj objectID]] longLongValue];
            sqlite3_bind_int64(statement, (int)(idx + 1), number);
        }
...
- (void)parseExpression:(NSExpression *)expression
         ...
         inFetchRequest:(NSFetchRequest *)request
               operator:(NSDictionary *)operator
                operand:(id __autoreleasing *)operand
               bindings:(id __autoreleasing *)bindings {
...
#pragma clang diagnostic pop
    ...
    if (type == NSKeyPathExpressionType) {
        ...
        if ([property isKindOfClass:[NSRelationshipDescription class]]) {
                ...
                if (rel.destinationEntity.superentity != nil) {
                    value = [value stringByAppendingString:
                             [NSString stringWithFormat:@" AND [%@]._entityType = %u",
                              rel.name,
                              (unsigned)rel.destinationEntity.name.hash]];
                }
                value = [value stringByAppendingString:@")"];
                foundPredicate = YES;
            }
            ...
            if(!foundPredicate) {
                ...
                __block id objectProperty = nil;
                [pathComponents enumerateObjectsUsingBlock:^(NSString * comp, NSUInteger idx, BOOL * stop) {
                    objectProperty = [subProperties objectForKey:comp];
                    if ([objectProperty isKindOfClass:[NSRelationshipDescription class]]) {
                        NSEntityDescription * entityDescription = [objectProperty destinationEntity];
                        subProperties = entityDescription.propertiesByName;
                    } else {
                        objectProperty = nil;
                        *stop = YES;
                    }
                ...
                if ([objectProperty isKindOfClass:[NSRelationshipDescription class]]) {
                    lastComponentName = [self foreignKeyColumnForRelationship:objectProperty];
                }
                value = [NSString stringWithFormat:@"%@.%@",
    ...
    // a value to be bound to the query
        ...
        if ([value isKindOfClass:[NSSet class]]) {
            NSUInteger count = [(NSSet*)value count];
